...
export default function ProdutoItem(props: ProdutoItemProps) {
  ...
  return (
    <Link
      ...
      className="
        flex flex-col bg-violet-dark border border-white/10 rounded-xl relative max-w-[350px]">
      <div className="h-48 w-full relative">
        <Image src={produto.imagem} fill className="object-contain" alt="Imagem produto" />
      </div>
      <div className="flex-1 flex flex-col p-5 gap-3 border-t border-white/10">
        ...
        <button className="flex bg-violet-700 rounded-full h-8 gap-2 justify-center items-center hover:border-2 border-emerald-500" onClick={(e) => {
          e.preventDefault();
